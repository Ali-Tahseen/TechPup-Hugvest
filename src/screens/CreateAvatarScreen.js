...
import { db, auth, storage } from '../functions/firebaseConfig';
...
function CreateAvatarScreen({ navigation }) {
  ...
  const uploadImage = async (uri, userId) => {
    try {
      ...
      const storageRef = ref(storage, `dog_images/${userId}/${Date.now()}`);
      await uploadBytes(storageRef, blob);
      const downloadURL = await getDownloadURL(storageRef);
      return downloadURL;
    } catch (error) {
      console.error('Image upload failed:', error.message, error.stack);
      throw error;
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      let photoURL = '';
      if (photo) {
        photoURL = await uploadImage(photo, dogID);
      }
      ...
      navigation.navigate('Home'); // Use navigate here instead of replace
    } catch (error) {
